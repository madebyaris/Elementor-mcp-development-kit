jQuery(document).ready(function($) {
	...
	const $selectAll = $('#ayu-select-all-scopes');
	const $scopeCheckboxes = $('.ayu-scope-checkbox');
	...
	// Create token form
	$('#ayu-create-token-form').on('submit', function(e) {
		e.preventDefault();
		...
		$.ajax({
			url: ayuAdmin.ajaxUrl,
			type: 'POST',
			data: {
				action: 'ayu_create_token',
				nonce: ayuAdmin.nonce,
				scopes: scopes,
				expires: expiresTimestamp,
			},
			success: function(response) {
				if (response.success) {
					$('#ayu-token-display').val(response.data.token);
					$('#ayu-token-modal').show();
				} else {
	...
	// Copy token
	$('#ayu-copy-token').on('click', function() {
		const tokenInput = $('#ayu-token-display');
		tokenInput.select();
	...
	// Close modal
	$('.ayu-modal-close').on('click', function() {
		$('.ayu-modal').hide();
		location.reload(); // Reload to show new token in list
	});

	// Revoke token
	$('.ayu-revoke-token').on('click', function() {
